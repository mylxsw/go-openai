...
var (
	...
	errorPrefixCompatible = []byte(`data:{"error":`)
)
...
func (stream *streamReader[T]) processLines() ([]byte, error) {
	...
	for {
		...
		if bytes.HasPrefix(noSpaceLine, errorPrefix) || bytes.HasPrefix(noSpaceLine, errorPrefixCompatible) {
			hasErrorPrefix = true
		}
		if !bytes.HasPrefix(noSpaceLine, headerData) || hasErrorPrefix {
			if hasErrorPrefix {
				noSpaceLine = bytes.TrimSpace(bytes.TrimPrefix(noSpaceLine, headerData))
			}
